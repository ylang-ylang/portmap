...
function splitDnsSetupCommand(catalog) {
	return `DNS_SERVER=${shellSingleQuote(catalog.dns_server)}
DNS_DOMAIN=${shellSingleQuote(catalog.dns_domain)}
...
function buildCatalogTree(catalog) {
	...
	for (const service of catalog.services || []) {
		...
		if (!project.branches.has(branchName)) project.branches.set(branchName, {
			branch: branchName,
			services: []
		});
		project.branches.get(branchName).services.push(service);
	}
	return Array.from(projects.values()).sort((left, right) => text(left.repo_name).localeCompare(text(right.repo_name)) || text(left.repo_id).localeCompare(text(right.repo_id))).map((project) => ({
		...project,
		branches: Array.from(project.branches.values()).sort((left, right) => left.branch.localeCompare(right.branch)).map((branch) => ({
			...branch,
...
function endpointExternal(endpoint) {
	...
	if ((endpoint.kind === "tcp" || endpoint.kind === "udp") && endpoint.host && endpoint.host_port) return `${endpoint.host}:${endpoint.host_port}`;
	if (endpoint.kind === "range" && endpoint.host && endpoint.host_port) {
		let rangeText = "";
		if (endpoint.range_start && endpoint.range_end) rangeText = ` relay ${endpoint.range_start}-${endpoint.range_end}`;
...
function showActionMessage(message, failed) {
	...
	if (!element) {
		...
		if (meta && meta.parentNode) meta.parentNode.insertBefore(element, meta.nextSibling);
	}
...
function branchSummaryStats(branch) {
	return {
		endpointTotal: branch.services.reduce((total, service) => total + endpointCount(service), 0),
		worktrees: uniqueSorted(branch.services.map((service) => service.worktree))
	};
}
function renderBranch(branch) {
	...
	for (const service of branch.services) for (const row of renderServiceRows(service)) tbody.appendChild(row);
...
function renderProject(project) {
	...
	for (const branch of project.branches) body.appendChild(renderBranch(branch));
...
function renderCatalogTree(catalog) {
	...
	for (const project of projects) container.appendChild(renderProject(project));
}
